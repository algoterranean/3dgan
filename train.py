...
from util.debug import format_for_terminal, message
...
def update_moving_average(status, moving_avg, pbar):
    ...
    pbar.set_postfix(format_for_terminal(moving_avg))



class load_args_from_file(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        # group arguments into list, and add '--' prefix
        contents = values.read().strip().split('\n')
        pair_list = []
        for i in range(len(contents)):
            kv_pair = contents[i].split()
            pair_list.append('--' + kv_pair[0])
            if len(kv_pair) == 2:
                pair_list.append(kv_pair[1])
        # parse
        data = parser.parse_args(pair_list, namespace=namespace)
...
if __name__ == '__main__':
    ...
    add('--skip_layers',
        action='store_true',
        default=False,
        help="""Use skip layers (if supported).""")
    ...
    try:
        with sv.managed_session(config=session_config) as sess:
            ...
            for epoch in range(current_epoch, max_epochs):
                ...
                for i in pbar:
                    if sv.should_stop():
                        ...
                        update_moving_average(status, running_total, pbar)

                        # record 10 extra summaries (per epoch) in the first 3 epochs,
                        # and record a summary half-way through each epoch after that
                        if (epoch < 3 and i % int((iter_per_epoch / 10)) == 0) or \
                                (epoch >= 3 and i % int((iter_per_epoch / 2)) == 0):
                            sv.summary_computed(sess, sess.run(summary_op, feed_dict={train_phase: data.TRAIN}))
                ...
                for i in vpbar:
                    ...
                    update_moving_average(status, running_total, vpbar)
            ...
            for i in tpbar:
                ...
                update_moving_average(status, running_total, tpbar)
